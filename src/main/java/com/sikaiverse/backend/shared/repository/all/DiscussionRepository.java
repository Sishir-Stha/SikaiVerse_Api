...
public interface DiscussionRepository extends JpaRepository<DiscussionEntity,Long> {
    ...
    @Query(value = "SELECT * FROM public.all_course_discussions( :courseId );",nativeQuery = true)
    ...
    @Query(value = "SELECT all_insert_discussion_post( :courseId,:userId, :title, :content );",nativeQuery = true)
    Boolean addPostReply (@Param("courseId") int courseId,
                          @Param("userId") int userId,
                          @Param("title") String title,
                          @Param("content") String content);

    @Query(value = "SELECT all_insert_discussion_reply( :postId, :userId, :content)",nativeQuery = true)
